...
pvalue_bh <- data.frame(p_value = p.adjust(pvalue$p_value, method = "BH")) # With ChatGPT
rownames(pvalue_bh) <- rownames(pvalue)
...
names_sig_genes <- rownames(sig_pval_bh)
...
# Subsetting by logFC for differential expression
logFC_up <- subset(enrichment_data, enrichment_data$logFC > 1)
logFC_down <- subset(enrichment_data, enrichment_data$logFC < -1)

# Creating a character vector for the KEGG analysis universe parameter
count_gene_vector <- count_geneID$GeneID
count_gene_vector <- as.character(count_gene_vector)
...
kegg_up <- enrichKEGG(gene = logFC_up$GeneID, 
                      ...
                      pvalueCutoff = 0.05, 
                      pAdjustMethod = "BH",
                      universe = count_gene_vector)

# Plotting KEGG enrichment analysis for up-regulated pathways
dotplot(kegg_up,
        showCategory = 10,
        title = "Top KEGG enriched up-regulated pathways")
...
# Performing KEGG enrichment for down-regulated genes
kegg_down <- enrichKEGG(gene = logFC_down$GeneID, 
                        organism = "hsa",
                        keyType = "kegg",
                        pvalueCutoff = 0.05, 
                        pAdjustMethod = "BH",
...
# Plotting KEGG enrichment analysis for down-regulated pathways
dotplot(kegg_down,
        showCategory = 10,
        title = "Top KEGG enriched down-regulated pathways")

# Saving plot as .png
ggsave("7_KEGG_down-regulated.png",
       plot = last_plot(),
       path = "Plots/",
       width = 8,
       height = 8)
